...
class TestDeviceModel:
    ...
    def test_compute_status_online_recent(self, app):
        db = SessionLocal()
        d = Device(device_id="esp32-recent", last_seen=datetime.now(UTC) - timedelta(seconds=5))
        db.add(d)
        db.commit()
    ...
    def test_compute_status_offline_expired(self, app):
        db = SessionLocal()
        d = Device(device_id="esp32-expired", last_seen=datetime.now(UTC) - timedelta(seconds=60))
        db.add(d)
        db.commit()
        assert d._compute_status() == "offline"
        db.close()
    ...
    def test_compute_status_naive_datetime(self, app):
        db = SessionLocal()
        d = Device(device_id="esp32-naive", last_seen=datetime.now() - timedelta(seconds=5))
        db.add(d)
        db.commit()
        assert d._compute_status() == "online"
        db.close()

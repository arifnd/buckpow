...
class TestMeasurementCreateSchema:
    ...
    def test_with_firmware(self):
        m = MeasurementCreate(
            device_id="esp32-01", bus_voltage=5.0, shunt_voltage=0.1, current=100, power=500, firmware_version="1.0.0"
        )
    ...
    def test_wrong_type(self):
        with pytest.raises(ValidationError):
            MeasurementCreate(
                device_id="esp32-01", bus_voltage="not_a_number", shunt_voltage=0.1, current=100, power=500
            )

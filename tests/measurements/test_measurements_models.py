...
class TestMeasurementModel:
    def test_to_dict(self, app):
        ...
        db.flush()
        m = Measurement(
            device_id=d.id, bus_voltage=5.0, shunt_voltage=80.0, load_voltage=5.08, current=0.24, power=1.2, energy=0.5
        )
        db.add(m)
        db.commit()
        data = m.to_dict()
    ...
    def test_measurement_repr(self, app):
        db = SessionLocal()
        m = Measurement(device_id=1, bus_voltage=5.0, shunt_voltage=80.0, load_voltage=5.08, current=0.24, power=1.2)

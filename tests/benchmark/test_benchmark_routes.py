class TestBenchmarkCompare:
    ...
    def test_compare_valid(self, client, app):
        ...
        MeasurementService(db).create("esp32-bench", bus_voltage=5.0, shunt_voltage=80.0, current=200, power=1000)
        MeasurementService(db).create("esp32-bench", bus_voltage=5.0, shunt_voltage=80.0, current=200, power=1000)
    ...
    def test_compare_three_sessions(self, client, app):
        ...
        MeasurementService(db).create("esp32-bench3", bus_voltage=5.0, shunt_voltage=80.0, current=200, power=1000)
        MeasurementService(db).create("esp32-bench3", bus_voltage=5.0, shunt_voltage=80.0, current=200, power=1000)
        MeasurementService(db).create("esp32-bench3", bus_voltage=5.0, shunt_voltage=80.0, current=200, power=1000)
    ...
    def test_compare_non_numeric_skipped(self, client, app):
        ...
        SessionService(db).start(s1.id)
        SessionService(db).start(s2.id)
        MeasurementService(db).create("esp32-bench2", bus_voltage=5.0, shunt_voltage=80.0, current=200, power=1000)
        s1_id, s2_id = s1.id, s2.id
        db.close()

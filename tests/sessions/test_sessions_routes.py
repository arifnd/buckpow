class TestSessionsAPI:
    ...
    def test_session_pagination(self, client, app):
        ...
        db.execute(insert(SessionModel), [{"device_id": device.id, "name": f"Session {i}"} for i in range(15)])
        db.commit()
        db.close()
    ...
    def test_session_page_zero_returns_all(self, client, app):
        ...
        db.execute(insert(SessionModel), [{"device_id": device.id, "name": f"Sess {i}"} for i in range(3)])
        db.commit()
        db.close()

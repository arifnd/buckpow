...
def db_info(current_user: User = Depends(require_user)):
    db_type = _detect_db_type()
    size = _get_db_size()
    tools = {'sqlite': True, 'postgresql': shutil.which('pg_dump') is not None, 'mysql': shutil.which('mysqldump') is not None}
    return {
        'type': db_type,
        'size': size,
        'backup_formats': {
...
def _backup_postgresql(ts: str):
    ...
    try:
        ...
        if proc.returncode != 0:
            raise HTTPException(status_code=500, detail=f'pg_dump failed: {proc.stderr.decode()}')
...
def _backup_mysql(ts: str):
    ...
    try:
        ...
        if proc.returncode != 0:
            raise HTTPException(status_code=500, detail=f'mysqldump failed: {proc.stderr.decode()}')
        compressed = gzip.compress(proc.stdout)
    except subprocess.TimeoutExpired:

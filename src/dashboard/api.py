...
def dashboard_data(db: Session = Depends(get_db), _current_user: User = Depends(require_user)):
...
def dashboard_summary(db: Session = Depends(get_db), _current_user: User = Depends(require_user)):
